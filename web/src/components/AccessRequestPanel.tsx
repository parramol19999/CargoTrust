      ...
      <div className="lg:col-span-1 card-light p-6 flex flex-col justify-between">
        <div>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider">Request Private Data</h3>
          </div>
          ...
          <form onSubmit={handleRequestAccess} className="space-y-3">
            ...
            {reqError && <div className="p-3 bg-red-50 text-red-600 border border-red-100 rounded-xl text-[11px] font-mono">{reqError}</div>}
      ...
      <div className="lg:col-span-2 card-light p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider">Access Sharing Desk</h3>
          </div>
        ...
        <div className="space-y-6">
          {/* Outgoing Requests */}
          <div>
            <span className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">My Requests</span>
            {myRequests.length === 0 ? (
              <span className="text-[11px] text-gray-400 italic">No access requests submitted by you.</span>
            ) : (
                ...
                {myRequests.map(req => {
                  ...
                  return (
                    <div key={req.id} className="p-3 bg-gray-50 border border-gray-150 rounded-xl flex items-center justify-between gap-4 text-xs">
                      <div>
                        <span className="font-bold text-gray-900 font-mono">Token #{req.tokenId}</span>
                        <div className="text-[10px] text-gray-400 font-mono mt-0.5">
                          Status: <span className={`font-bold ${req.status === 'Approved' ? 'text-emerald-600' : req.status === 'Pending' ? 'text-amber-600' : 'text-red-600'}`}>{req.status}</span>
                        </div>
                      </div>
          ...
          <div className="border-t border-gray-150 pt-4">
            <span className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Incoming Data Requests</span>
            {incomingRequests.length === 0 ? (
              <span className="text-[11px] text-gray-400 italic">No incoming requests to authorize.</span>
            ) : (
              <div className="space-y-2.5 max-h-[160px] overflow-y-auto pr-1">
