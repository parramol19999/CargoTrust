...
import WalletOverviewDashboard from '@/components/WalletOverviewDashboard';

export default function OperationsDesk() {
  const [refreshTrigger, setRefreshTrigger] = useState(0);
  const [activeTab, setActiveTab] = useState<'wallet' | 'desk' | 'escrow' | 'agents' | 'lending' | 'developer'>('wallet');
    ...
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      <main className="flex-grow max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-fade-in-up">
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div className="space-y-2">
              <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight text-slate-950">
                {isSimpleMode ? 'Secure Coffee Sourcing & Trades' : 'Crop Sourcing & Ownership Settlements'}
              </h1>
              <p className="text-sm text-slate-500 max-w-xl leading-relaxed">
                {isSimpleMode 
                  ? 'Create digital receipts for your crops, verify testing certificates from laboratories, and buy or sell safely using digital dollars (USDC).'
            ...
            <div className="grid grid-cols-2 sm:flex items-center gap-6 border-t md:border-t-0 md:border-l border-slate-100 pt-6 md:pt-0 md:pl-8 shrink-0 font-mono">
              <div className="text-left md:text-right">
                <span className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">
                  {isSimpleMode ? 'Network Fees' : 'Platform Gas'}
                </span>
                <p className="text-sm font-bold text-slate-900 mt-1">
                  {isSimpleMode ? 'Paid in Digital Dollars' : 'USDC Gas (Native)'}
                </p>
              </div>
              <div className="text-left md:text-right">
                <span className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">
                  {isSimpleMode ? 'Payment Protection' : 'Trading Model'}
                </span>
                <p className="text-sm font-bold text-emerald-600 mt-1">
                  {isSimpleMode ? 'Cash on Verification' : 'Atomic USDC Settlement'}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex border-b border-slate-200/80 gap-1.5 px-1 overflow-x-auto scrollbar-none">
          ...
          >
            {isSimpleMode ? '🌱 Treasury & Portfolio' : 'Treasury & Analytics'}
          </button>
          <button
          ...
          >
            {isSimpleMode ? '1. Trade & Provenance Desk' : 'Sourcing & Trading Desk'}
          </button>
          <button
          ...
          >
            {isSimpleMode ? '2. Secure Escrow Vault' : 'Escrow Manager'}
          </button>
          <button
          ...
          >
            {isSimpleMode ? '3. Automated Trackers' : 'Agent Registry'}
          </button>
          <button
          ...
          >
            {isSimpleMode ? '4. Purchase Finance' : 'Trade Credit'}
          </button>
          <button
          ...
          >
            {isSimpleMode ? '5. Technical Portal' : 'Developer Console'}
          </button>
        </div>

        {/* Dynamic Onboarding Explainer */}
        <FriendlyOnboarding section={activeTab} />
